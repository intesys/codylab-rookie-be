...
public class PatientApiTest {
    ...
    void getPatientTest() throws  Exception{
        Long id = 31L;
        String name = "Qua";
        String surname = "Pavarana";
        String email = "franco.pavarana@gmail.com";
        MvcResult result = mvc.perform(MockMvcRequestBuilders.get(PatientApi.API_PATIENT_ID, id)).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        String jsonResponse = result.getResponse().getContentAsString();
    ...
    void updatePatientTest() throws  Exception{
        ...
        mvc.perform(MockMvcRequestBuilders.put(PatientApi.API_PATIENT_ID, id).contentType(MediaType.APPLICATION_JSON).content(jsonRequest)).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
    ...
    void deletePatientTest() throws  Exception{
        ...
        mvc.perform(MockMvcRequestBuilders.delete(PatientApi.API_PATIENT_ID,id)).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        count = db.queryForObject("select count (*) from patient where id= ?", Integer.class, id);
        Assertions.assertEquals(0, count, "Patient doesn't exist");
    }
