...
public class PatientRecordMapper implements RookieMapper<PatientRecord, PatientRecordDTO> {
    ...
    public PatientRecord toEntity(PatientRecordDTO dto) {
        ...
        entity.setDoctor(Optional.ofNullable(dto.getDoctor()).map(DoctorDTO::getId).map(doctorRepository::findById).orElse(null));
        entity.setPatient(Optional.ofNullable(dto.getPatientId()).map(patientRepository::findById).orElse(null));
    ...
    public PatientRecordDTO toDTO(PatientRecord entity) {
        ...
        dto.setDoctor(Optional.ofNullable(entity.getDoctor()).map(doctorMapper::toDTO).orElse(null));
        dto.setPatientId(Optional.ofNullable(entity.getPatient()).map(Patient::getId).orElse(null));
